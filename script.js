...
// Round tracking and multipliers
let roundNumber = 1;
let currentRoundIndex = 0;
const multipliers = [1, 2, 3]; // Round 1 = x1, Round 2 = x2, Round 3 = x3
...
function renderFinalQuestions(container, prefix) {
    container.innerHTML = '';
    finalFeudQuestions.forEach((fq, idx) => {
...
function submitPlayer1() {
    player1Answers = [];
    finalFeudQuestions.forEach((q, idx) => {
...
function getPointsForFinal(questionIndex, answerValue) {
    ...
    const entry = finalFeudQuestions[questionIndex].answers.find(ans => ans.text.toLowerCase() === answerValue);
...
function showFinalResults() {
    ...
    let total = 0;
    for (let i = 0; i < finalFeudQuestions.length; i++) {
